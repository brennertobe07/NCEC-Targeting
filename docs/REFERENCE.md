...
Vantage is a single-file HTML application that displays NCEC targeting data for Virginia's 2026 election cycle. It provides interactive exploration of statewide, county, and precinct-level Democratic performance metrics, with district overlays (HD/SD/CD) and a targeting/flagging system.

**Live URL:** https://vantage.vadems.org

---

## ⚠️ Not to be confused with `va-elections`

There are two similarly-themed repos. Keep them straight:

- **`brennertobe07/Vantage`** (THIS repo) — the Vantage Election Explorer (NCEC targeting). Custom domain **vantage.vadems.org**.
- **`brennertobe07/va-elections`** — a *different* project: the **Virginia Candidate Explorer** (`index.html` + `data/` candidate/donor dataset). No custom domain.

Vantage work goes here. A stray copy of the Vantage app once lived in `va-elections` and caused confusion — that has been removed.
...
- **Custom domain:** `CNAME` file → `vantage.vadems.org`
...
| `gov21dem2way` | Governor 2021 Dem two-way % (historical) |
| `pres20dem2way` | President 2020 Dem two-way % (historical) |
...
Deploy model: commit `index.html` → GitHub Pages serves it (source: `main` branch, root path).

In Claude Code (preferred now):
1. Edit `index.html` (and regenerate data with `embed_ncec_data.py` if the NCEC source changed)
2. Commit and push to `main`
3. Site updates at vantage.vadems.org within 1-2 minutes (hard-refresh to bypass browser cache)

(GitHub Desktop also works for the commit/push step if preferred.)
